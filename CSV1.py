...
def getcsv(rel,columns):
    ...
    df.to_csv(path_or_buf="BBBBBB.csv",
    sep=',',
    encoding='utf-8',
    header=True,
    index=False)
...
if __name__ == '__main__':
    sql = '''select id, device_no, device_val, create_time from device_log 
    where device_no=12 and create_time> '2022-06-15 12:00:00' order by create_time desc;'''
    rel,columns = getrel(sql)
    getcsv(rel,columns)
    plot = ploting(file_path='BBBBBB.csv', time_col='create_time')
    plot.run()